...
| `customers.png` | 客24種、前後左右 | 4列×24行、160×220px/セル |
...
トラックドライバー・土方・ブルーカラー等の職業/属性アーキタイプを含む24種を
収録する。各行が1アーキタイプ、列は前後左右(`facingColumns`)。行とアーキタイプ
の対応は`customers-manifest.json`を参照する(座標を推測しないこと)。

現状の選択ロジックは`CUSTOMER_ROWS`から一様ランダムに選ぶだけで、アーキタイプ
ごとの出現確率(立地による差)や買い物思考の違いは未実装。これらの挙動面の
実装は別タスクとして扱う。
