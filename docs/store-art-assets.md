...
| `customers.png` | 客32種、前後左右 | 4列×32行、160×220px/セル |
...
トラックドライバー・土方・ブルーカラー等の職業/属性アーキタイプ(行0-23)、および
ChatGPT製アセットパックから採用した店舗制服風の客(行24-31、元は店員用素材)を含む
32種を収録する。各行が1アーキタイプ、列は前後左右(`facingColumns`)。行と
アーキタイプの対応は`customers-manifest.json`を参照する(座標を推測しないこと)。
...
(`UNMODELED_ARCHETYPE_WEIGHT_SHARE`)。行24-31(今回追加分)は、まだどのコホートにも
割り当てていないため、この一律フレーバー枠からのみ出現する。
