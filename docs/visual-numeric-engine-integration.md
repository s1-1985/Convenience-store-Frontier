...
1. 棚面積(`categoryArea`点数配分)とタイル什器レイアウトの統合(什器の物理配置
   そのものの統合。数値の同期自体は下記フェーズ2bで着手済み) — 下記
   「フェーズ2d」参照。「投資額→タイル数」の一方向のみ一部着手済み
...
## フェーズ2c: 実欠品シビアリティによる什器商品絵柄の表示バイアス(2026-09-03、着手)

上記フェーズ2の2「在庫欠品表示の同期」のうち、フェーズ2aで着手済みの「翌日納品量
への反映」に続き、「什器に描画される商品の絵柄(`merchandise.png`のfillRatio)」
にも同じ実欠品シビアリティ信号を反映した。ユーザーとの協議の結果、以下の2点を
明確にスコープとした:

- 対象は`resolveFixtureStockState`(`src/ui/storeArtAssets.ts`)が返す
  `FixtureStockState`(什器の商品絵柄の表示段階)のみ。
- `storeGameRuntime.ts`内の「品切れ/残り少」警告バッジと、フッターの
  カテゴリ別在庫バー(「N/24」等の数値表示)は、いずれも`inventory.shelfUnits`/
  `shelfCapacity`を直接参照する既存経路のまま**あえて変更しない**。この2つは
  「棚在庫が減る→空になる→警告が出る」というプレイヤーが画面上で直接確認できる
  因果関係を厳密に保つべき表示であり、実欠品シビアリティ(本物の`Simulation`側の
  信号)を混ぜると「棚の商品は残っているのに警告が出る/出ない」という見た目上の
  矛盾を生みかねないため。什器の商品絵柄は数を数えて確認するような表示ではない
  「雰囲気」の表現であり、本物の経済が欠品気味のカテゴリへ寄せても実害が薄いと
  判断した。この選択は2択目のAskUserQuestionでユーザーが明示的に選択したもの。

### 実装内容

- `StoreOperationsSnapshot`(`src/game/storeOperationsEngine.ts`)に
  `stockoutSeverityByCategory: Partial<Record<StoreCategoryId, number>>`を追加し、
  `getSnapshot()`が`beginDay()`に渡された実欠品シビアリティ(フェーズ2aで既存の
  内部変数)をそのまま公開するようにした。`serialize()`経由でセーブデータにも
  含まれるが、`restoreStoreOperationsEngine`側では読み戻さず、常に`{}`で初期化
  される(フェーズ2aと同じく「次回`beginDay()`で本物のSimulationから再計算される
  派生信号であり、それ自体を権威ある状態として永続化しない」という既存方針を
  踏襲)。
- `resolveFixtureStockState()`(`src/ui/storeArtAssets.ts`)を、素の
  `shelfUnits/shelfCapacity`比率(`rawRatio`)に加え、対象カテゴリの実欠品
  シビアリティ(0〜1にクランプ)を`REAL_STOCKOUT_DISPLAY_BIAS = 0.6`(最大60%)
  で乗じて減衰させるよう変更した。シビアリティ0または未定義(データ無し・
  シェアードセッション未読込)では従来と完全に同じ結果になる。
- `src/tests/storeArtAssets.test.ts`に3本追加(計13本):
  - シビアリティが無い/0のとき表示が変化しないこと
  - シビアリティが上がるにつれ表示段階が「欠品寄り」に遷移すること
    (半分容量の棚で`normal`→`low`への閾値越えを検証)
  - シビアリティが0〜1の範囲外(5や-3)でも、実装がクランプ処理を持つことで
    範囲外の値による負の比率・過大な比率を防いでいること

### 検証(実施済み)

- `npx tsc --noEmit -p .`: エラー無し
- `npx vitest run`: 184件全通過(既存181件 + 新規3件)
- `npm run balance:ci`: 変更前と数値的に完全に同一(この変更はCanvas描画層のみに
  影響し、バランスベンチマークが経由する`Simulation`/`storeCanvasPolicySync`等の
  数値エンジン層には触れていないため予想通り)
- Playwright: 開発サーバー上で店舗を開き、時間パネルから20倍速・再生を実行して
  複数日分を進行させ、コンソール・ページエラーが出ないこと、什器の商品絵柄・
  レジ待ち警告・フッター在庫バーが正常に描画されることをスクリーンショットで確認

### 残課題(未着手のまま)

- 「品切れ/残り少」警告バッジとフッター在庫バーは、上記の通り意図的に対象外の
  ままであり、フェーズ2の2「商品単位とカテゴリ単位の完全統合」自体はまだ
  部分的にしか達成されていない(什器の商品絵柄という表示レイヤーのみ)
- 商品単位バッチとCanvasのカテゴリ単位在庫(`shelfUnits`自体)の完全統合は
  引き続き未着手(フェーズ2aから継続する既知の残課題)
- フェーズ2の1(棚面積とタイル什器レイアウトそのものの統合)は、下記
  「フェーズ2d」参照。3(スロット単位で来店客1人1人を数値エンジンと厳密に
  対応させる演出)は、今回のユーザー判断により明示的に見送り継続

## フェーズ2d: フェーズ2の1「棚面積とタイル什器レイアウトの統合」の現状整理(2026-09-03)

フェーズ2c着手後、ユーザーからフェーズ2の残課題のうち1「棚面積
(`categoryArea`点数配分)とタイル什器レイアウトの完全統合(什器の物理配置
そのものの統合)」を深掘りするよう指示があった。実装前に既存コードを調査した
ところ、この項目は**ADR-0006(店舗の物理拡張メカニクス、PR #72、2026-09-02)に
よって一方向(投資額→タイル数)がすでに部分的に実装済み**であることが判明した。
本ドキュメントの一覧がこれを反映しておらず古いままだったため、今回は以下の
現状整理のみ行い、実装はユーザー判断により見送った。

### 現状(ADR-0006で実装済みの範囲)

- `investInCategoryCapacity()`(`storeOperationsEngine.ts`)の設備投資は3段階
  (`SHELF_TIER_COSTS`、+6ずつ、`shelfCapacity`のみ増加)に加え、4段階目
  「拡張工事」(`PHYSICAL_EXPANSION_COST`、+10)を持つ。4段階目のみ
  `applyPhysicalExpansion()`が該当什器の`StoreFixture.tiles`を実際に1行分
  増やし、接客ポイントも新しい行へ移設する(`PHYSICAL_EXPANSION_DIRECTIVES`、
  カテゴリごとに個別定義)。9カテゴリ全てが対応済み(`maxShelfTier()`が全て4を
  返す)
- この`shelfCapacity`は、フェーズ2bの`categoryAreaFromShelfCapacity()`により
  引き続きSimulation側の`categoryArea`へ比例配分されるため、4段階目投資による
  タイル拡張は数値エンジン側の棚面積配分にも(既存の比例配分ロジックを通じて)
  反映される。つまり「投資額→タイル数拡張→categoryArea配分の増加」という
  一方向の統合は現状で成立している

### 未着手のまま残る範囲

- **逆方向(タイル数→投資額/categoryArea)が無い**: 売場配置編集
  (`storeLayoutEditor.ts`/`storeLayoutEditorUi.ts`)は什器の移動・回転のみ
  対応しており、タイル数を変える(拡大・縮小する)操作自体が存在しない。
  よってプレイヤーが手動でレイアウトを組み替えても`shelfCapacity`/
  `categoryArea`には一切影響しない
- 4段階目の拡張は各カテゴリにつき固定1回・固定1行分(`PHYSICAL_EXPANSION_
  DIRECTIVES`で座標を決め打ち)であり、任意のタイル数へ連続的に拡張できる
  仕組みではない
- 拡張後の什器アート(什器タイルの拡大表示)がPlaywrightで実機確認された
  という記録は見当たらない(ADR-0006のConsequencesでも「未確認」と明記)

「什器の可変サイズ編集をレイアウトエディタへ追加し、その結果をshelfCapacity/
categoryAreaへ反映する」という本格的な双方向統合は、可変サイズ編集・セーブ
...
## フェーズ2f: 通知の原因階層(`docs/game-design.md` §10)をアラートへ付与(2026-09-03)

ユーザーから「ゲームデザインを進めて」との指示を受け、`docs/game-design.md`10節
「UI原則」を確認したところ、以下の仕様が既に承認済みだが未実装のまま残っていた。

> 通知は問題を示すだけでなく、原因階層を持つ。
> 売上低下 → 昼の来店減少 → 弁当欠品増加 → 午後便遅延と補充不足

フェーズ2e時点の`buildDashboardAlerts()`は「レジ待ちで多数離脱」のような単発の
症状しか出しておらず、この仕様の「原因階層」を満たしていなかった。ユーザーへ
実装範囲を確認したところ「既存アラートの因果連鎖化(推奨)」が選ばれた
(新しい数値・バランス変更は伴わない範囲)。

### 実施内容

- `DashboardAlert`(`presentation.ts`)に`causeChain?: string[]`を追加。
  `buildDashboardAlerts()`が生成する7種のアラート(大幅な赤字/赤字営業、
  レジ待ちで多数離脱/レジ待ちが発生、補充遅延で棚が空いている/棚補充が
  追いついていない、在庫そのものが不足/商品在庫が不足、廃棄負担が大きい、
  店舗作業が積み残されている、常連客が競合へ流出)全てに、`DailyReport`に
  既存のフィールドだけから1段階分の原因を追加した:
  - 赤字系: 人件費/廃棄コスト/物流費/光熱費のうち最も利益を圧迫している費目
    (`dominantCostCause()`)
  - レジ待ち系: `operationBacklogByTask.register`(レジ業務の未処理蓄積か、
    未処理は少なく人員配置の問題か)
  - 棚補充系: `operationBacklogByTask.replenishment`(同様)
  - 在庫不足系: `backroomInventoryUnitsEnd`とストックアウト数の比較
    (バックヤード自体が不足か、補充が追いついていないだけか)
  - 廃棄負担: 欠品も同時発生しているか(発注のカテゴリ配分の偏り)、
    欠品はほぼ無く発注過多の可能性が高いか
  - 作業積み残し: `operationBacklogByTask`のうち最大のタスク
    (`largestOperationTask()`、`OPERATION_LABELS`で日本語化)
  - 常連客流出: 欠品と行列のどちらの影響がより大きいか
- `storeGameRuntime.ts`の`renderDayAlert()`が、`alert.detail`の後ろに
  `causeChain`の各段を「→ 」区切りで連結して表示するよう変更。`storeGame.css`
  の`.day-alert`を単一行省略表示から複数行折り返しへ変更(`flex-direction:
  column`、`white-space: normal`)
- `src/tests/ui.presentation.test.ts`に3本追加(レジ待ちアラートの原因が
  レジ業務backlogを指すこと、在庫不足アラートの原因がバックヤード在庫量に応じて
  切り替わること、作業積み残しアラートの原因が最大backlogタスクを名指しすること)
- 商品カテゴリ別・時間帯別の粒度(仕様例の「昼の来店減少」「弁当欠品増加」に
  相当)は、現状`buildDashboardAlerts()`に`scenario.products`や時間帯別来店数
  が渡されておらず、関数シグネチャの変更を伴うため、今回の「既存データのみで
  完結させる」というユーザー判断のスコープ外として見送った

### 検証

- `npx tsc --noEmit -p .`: エラー無し
- `npx vitest run`: 188件全通過(既存185件 + 新規3件)
- Playwright: 20倍速で放置し、「本日:在庫そのものが不足 商品在庫不足による
  欠品が317.9個分発生した。発注方針または納品方式を見直す。 →
  バックヤード在庫自体が不足している(残り71点)」という2段階の原因階層が
  Canvas上のバナーに折り返し表示され、他の要素と重ならないことを
  スクリーンショットで確認

### 残課題(未着手のまま)

- カテゴリ・時間帯レベルの粒度(仕様例そのものの再現)は、`buildDashboardAlerts()`
  へ`scenario.products`等を渡すシグネチャ変更を伴うため未着手
- 原因階層は現状1段階のみ(仕様例は3段階)。2段階目以降を追加するかどうかは
  今回判断していない
