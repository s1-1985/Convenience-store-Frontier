...
describe("playtest session", () => {
  ...
  it("30日完走、45〜70分、判断6回、視覚発見率60％を判定する", () => {
    ...
    const issueIds: readonly PlaytestIssueId[] = ["queue", "empty_shelf", "closed_demand"];
  ...
  it("5セッション以上かつ60％以上のテスターで視覚発見条件を満たすと集計根拠が揃う", () => {
    const sessions = Array.from({ length: 5 }, (_, index) => {
      ...
      const detections: readonly PlaytestIssueId[] =
        index < 3 ? ["queue", "empty_shelf", "closed_demand"] : ["queue"];
      for (const issueId of detections) {
        session = recordPlaytestVisualDetection(session, issueId, {
          day: 1,
          timeLabel: "10:00",
