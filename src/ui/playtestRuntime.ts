...
function recordValues(selector: string): Record<string, number> {
...
function readPolicySnapshot(): PlaytestPolicySnapshot {
  ...
  return {
    ...
    staffing: recordValues("#staffing-controls input"),
    taskPriorities: priorities,
    categoryArea: recordValues("#category-area-controls input"),
  };
}
...
function writeStorage(key: string, value: unknown): void {
  try {
    ...
    // Private browsing and embedded previews may deny storage. The live session still works.
  }
}
...
function finishCurrent(reason: PlaytestFinishReason): void {
  ...
  archive(session);
  completedArchived = reason === "completed";
  writeStorage(ACTIVE_STORAGE_KEY, session);
}
...
function recoverInterruptedSession(): void {
  ...
  if (isPlaytestSession(stored) && !stored.finished) {
    const closed = finishPlaytestSession(stored, "reloaded", Date.now());
    archive(closed);
  }
...
function formatDuration(minutes: number): string {
  ...
  return `${whole}分${seconds.toString().padStart(2, "0")}秒`;
}
...
function renderPanel(): void {
  ...
  const detectionButtons = PLAYTEST_ISSUE_IDS.map(
    (issueId) => `
      ...
      >
        ${detected.has(issueId) ? "✓" : "+"} ${escapeHtml(ISSUE_LABELS[issueId])}
      </button>
    `,
  ).join("");
  ...
  host.innerHTML = `
    <div class="playtest-heading">
      <div>
        <p class="eyebrow">LOCAL PLAYTEST LOG</p>
        <h2>試作計測</h2>
      </div>
    ...
    <div class="playtest-metrics">
      ...
      <div><span>数値前の気づき</span><strong>${percent(assessment.visualDiscoveryRate)}</strong></div>
    </div>
    <section class="playtest-detection-section">
      <div class="playtest-section-title">
        <h3>店内表示だけで気づいた</h3>
        <small>数値レポートを見る前に押す</small>
      </div>
    ...
    <div class="playtest-history-summary">
      <span>保存セッション ${aggregate.sessionCount}件</span>
      <span>視覚発見合格 ${percent(aggregate.visualTesterPassRate)}</span>
...
function moment(): { day: number; timeLabel: string; nowEpochMs: number } {
...
function capturePolicyDecision(): void {
  ...
  session = recordPlaytestPolicyDecision(session, readPolicySnapshot(), moment());
  persist();
  queueRender();
}
...
function recordCampaignFromDom(acknowledged = false): void {
  if (!session || session.finished) return;
  const event = document.querySelector<HTMLElement>("#campaign-panel .campaign-event:not(.empty)");
  const title = event?.querySelector("h3")?.textContent?.trim();
  if (!event || !title) return;
  const button = event.querySelector<HTMLButtonElement>("[data-event-id]");
  const id = button?.dataset.eventId || `campaign-${title}`;
  const dayText = event.querySelector<HTMLElement>(".campaign-event-meta span")?.textContent ?? "";
  session = recordPlaytestCampaignCheckpoint(
    session,
    { id, title, day: Math.max(0, Math.round(parseNumber(dayText))), acknowledged },
    Date.now(),
  );
}
...
function synchronizeDomState(): void {
  ...
  if (stopReason.startsWith("自動停止") && stopReason !== lastAutoStopReason) {
    ...
    session = recordPlaytestAutoStop(session, stopReason, moment());
  }
  captureFinalEvaluation();
  const finished = session.completedDay >= 30 || optionalElement("play-button")?.textContent?.includes("営業完了");
  if (finished && !session.finished) finishCurrent("completed");
  if (session.finished && session.finishReason === "completed" && !completedArchived) {
    archive(session);
    completedArchived = true;
  }
  persist();
  queueRender();
}
...
function handleClick(event: MouseEvent): void {
  ...
  if (issueButton && session && !session.finished) {
    ...
    if (issueId && PLAYTEST_ISSUE_IDS.includes(issueId)) {
      session = recordPlaytestVisualDetection(session, issueId, moment());
      persist();
      queueRender();
    }
    return;
  ...
  if (!button) return;
  const control = CONTROL_BY_ID[button.id];
  if (control) recordControl(control);

  if (button.id === "apply-policy-button") {
    window.setTimeout(capturePolicyDecision, 0);
  }
