...
## 0-U. 続くセッション(2026-09-04、実機プレイのバグ2件を修正: フッターの表示崩れ・旧セーブの冷凍/ホット欠損)

ユーザーがAndroidデモAPKを実機で操作し、フッターに「冷凍undefined/undefined
ホットundefined/undefined」という壊れた表示が出て、中央の赤い日次診断バナーが
消えず時間も客も進まないように見える、というスクリーンショット付きの報告があった。
調査の結果、**互いに独立した2件の実バグ**であることが判明した(「時間が進まない」
自体は、後述の通りバグではなく既存の自動停止機能の挙動と判断)。

### バグ1: フッターのカテゴリ在庫バーが経営目標パネルと重なる(常に再現)

`docs/backlog.md`のADR-0003(冷凍食品)・ADR-0004(ホットスナック)でカテゴリが
7種→9種に増えたのに、`drawFooter()`(`storeGameRuntime.ts`)のレイアウト計算
(`14 + index * 118`の1段組み)が据え置きのままだった。9カテゴリ目・8カテゴリ目
(index 7・8、x=840〜1063)が、固定位置の「本日の経営目標」パネル
(`objectiveX=842`〜)と正面から重なり、冷凍食品・ホットスナックの数値が
読めなくなっていた(これはセーブデータの新旧に関係なく**常に**発生するレイアウト
バグで、実機で報告された画面はこれと下記バグ2が重なって出ていた)。

- 5列×2段に折り返す(`FOOTER_CATEGORY_COLUMNS = 5`)よう`drawFooter()`を修正。
  1段目5カテゴリはx=14〜604、2段目4カテゴリもx=14〜486で、いずれも
  経営目標パネル(x≥842)の手前に収まる
- Playwrightで実機確認: 修正前は7段目以降が経営目標パネルへ食い込んでいたのが、
  2段組みで完全に分離されることをスクリーンショットで確認

### バグ2: ADR-0003/0004以前のセーブを復元すると冷凍/ホットの在庫が空オブジェクトになる

`copyInventory()`(`storeOperationsEngine.ts`)が`{ ...inventories[categoryId] }`と
旧セーブの値をそのままスプレッドしていたため、冷凍食品・ホットスナックが
存在しなかった時期に保存されたセーブを復元すると、`inventories["frozen"]`/
`["hot"]`が`undefined`となり、`{ ...undefined }`は例外を投げず空オブジェクト`{}`に
なる。結果、`shelfUnits`/`shelfCapacity`/`price`などが全て`undefined`のまま
残り、フッターに文字通り「undefined/undefined」と表示されていた
(`daysSincePolicyChange`のcash/categoryTiersフォールバック実装〈2.5節参照〉と
同種の、新フィールド追加時の後方互換漏れ)。**さらに深刻な点として**、この状態で
「商品」パネルを開くと`inventory.price.toLocaleString()`(`renderProductPanel()`)が
`undefined.toLocaleString()`となり例外を投げ、そのクリックハンドラ内の処理が
そこで止まる(パネルが開かない)ことも確認した(ただし本体の`requestAnimationFrame`
描画ループとは別経路のため、この一点だけでは「時間が進まない」の説明にはならない)。

- `copyInventory()`を`{ ...CATEGORY_DEFAULTS[categoryId], ...inventories[categoryId], categoryId }`
  に変更。旧セーブに無いフィールドは`CATEGORY_DEFAULTS`(ADR-0003/0004で追加済みの
  冷凍・ホット用デフォルト値)で補い、旧セーブに実際にある値は上書きで尊重する
- 回帰テスト`src/tests/storeOperationsEngine.test.ts`に1本追加(`source.inventories`から
  `frozen`/`hot`キーを`delete`した状態から復元し、数値が全て有限値になることを検証)。
  計190件全通過(既存189件+新規1件)

### 「時間が進まない・客が来ない・赤いバナーが消えない」について(バグではないと判断)

ユーザー報告のスクリーンショットでは営業状態が「一時停止」だった。コードを確認した
限り、これは`autoStopForLatestReport()`(`main.ts`)——「重大問題を検出したら
自動停止」チェックボックス(既定でON)が、日替わり時に生成された日報が
critical/warningアラートを含むと自動的に▶を止める機能——が、報告された深刻な
バックヤード在庫不足を検出して発火した結果だと考えられる(推論: この特定セッションの
実機ログは見ていないため断定はできないが、コード上の唯一の自動一時停止経路であり、
症状〈一時停止表示・赤いバナーの内容が正にその不足を指している〉と整合する)。
日次診断バナー(`renderDayAlert()`、0-N/フェーズ2e)はその日の最新診断を表示し続ける
仕様で、次の日が確定するまで消えない設計になっており、「ダイアログ」のように見えるが
閉じるボタンを持たない常設ステータス表示である。▶(再生/停止)を時間パネルから
押せば再開でき、それ自体は既存の1倍/4倍/20倍の速度切り替えと合わせて実装済みだが、
ユーザーへの返信で明示的に説明した(コード変更は行っていない)。UIとして「一時停止中
である」「▶で再開できる」をもっと分かりやすくする改善の余地はあるが、今回は
指摘された2件のバグ修正に留め、この点はユーザーへの説明のみとした

### 検証

- `npx tsc --noEmit -p .`: エラー無し
- `npx vitest run`: 190件全通過(既存189件+新規1件)
- Playwright: フッター2段組みが実際に経営目標パネルと重ならず、冷凍食品
  10/14・ホットスナック8/12のように実数値で表示されることをスクリーンショットで
  確認(新規セーブでの確認。旧セーブ欠損シナリオはvitestの回帰テストでのみ確認、
  ブラウザでの再現は行っていない)

### 残っている作業 → 0-Vで一部対応(バナーに閉じるボタンを追加)

~~「一時停止中である」「▶で再開できる」をより分かりやすく示すUI改善(例:
日次診断バナーに再開方法のヒントを添える等)は今回着手していない。ユーザーの
反応次第で次候補にする~~

## 0-V. 続くセッション(2026-09-04、通知バナーに閉じる(×)ボタンを追加)

0-Uをマージ・APK再ビルド後、ユーザーから改めて実機スクリーンショット付きで
「画面中央の赤い吹き出し消えないんだけど。さっきも言ったのに治ってない」との
報告があった。0-Uでは`renderDayAlert()`/`renderLiveIncident()`のバナーは
「問題が解決するまで表示し続ける」仕様だと**説明のみ**行い、コードは変更して
いなかった。この報告を受けて再調査し、以下を発見・修正した。

### 発見1: `.live-incident`(両バナーの共通クラス)が`pointer-events: none`だった

`storeGame.css`を確認したところ、バナー自体がクリックを一切受け付けない
設定になっており、**そもそも閉じる手段がコード上に存在しなかった**
(ユーザーが「消せない」と感じたのは主観ではなく事実だった)。

### 発見2: リアルタイムの重大インシデントでも自動一時停止する経路が別にあった

0-Uでは`main.ts`の`autoStopForLatestReport()`(日替わり時の自動停止)のみを
原因として説明していたが、`renderLiveIncident()`(`storeGameRuntime.ts`)にも
**同じ「重大問題を検出したら自動停止」チェックボックスを見て、行列・欠品等の
リアルタイム重大インシデントで`play-button`を自動クリックする、独立した
もう一つの自動停止経路**があることに今回気づいた(0-Uの調査では見落としていた)。
実機画面が「レジ行列19人待ち」の重大インシデント表示中に一時停止していたのは、
主にこちらが原因だった可能性が高い(推論: 実機のログは見ていないため断定は
できないが、コード上この経路以外に一時停止を自発的に起こす箇所は無い)。
なお`incident.id`(カテゴリ単位の固定文字列)で再発火を1回に制限するガードが
既にあるため、「▶を押すたびに即座に再度停止する」無限ループではなく、
「一度止まったら、問題が一旦解消して再発するまで再度は止めない」という
動作であることをコードで確認した。

### 実施内容

- `.banner-dismiss`ボタン(×)を両バナーのHTMLテンプレートに追加。
  `pointer-events: auto`で個別に上書きし、絶対配置で常に右上に固定する
  (`.live-incident`の1行レイアウトと`.day-alert`の縦積みレイアウトの両方で
  同じ見た目になるよう、flexの並び順ではなく`position: absolute`を採用)
- `renderLiveIncident()`: `dismissedLiveIncidentId`(モジュール変数)と
  `incident.id`が一致する間は`banner.hidden = true`のまま表示しない。自動停止
  自体(`optional("play-button")?.click()`)は閉じたかどうかに関わらず従来通り
  発火する(表示上の好みと、安全機構としての自動停止は独立させた)。インシデント
  自体が解消(`detectStoreIncidents`が空配列)すれば`dismissedLiveIncidentId`も
  リセットされ、次に別カテゴリの問題が起きれば通常通り表示される
- `renderDayAlert()`: 同様に`dismissedDayAlertDay`(日報の`day`番号)で管理。
  `latestDayAlert()`の戻り値を`DashboardAlert`単体から`{ day, alert }`へ変更し、
  どの日の診断を閉じたかを追跡できるようにした。次の日の診断に切り替われば
  自動的に再表示される(同じ日の同じ問題を毎フレーム主張し続けるのを止める
  だけで、握りつぶしではない)
- クリックハンドラを`shell`の既存イベント委譲(`data-close-staff`等と同じ
  パターン)に追加
- `src/tests/storeGameShell.test.ts`に1本追加(計7本、他ファイルと合わせ
  全体190件全通過)
- `npx tsc --noEmit -p .`・`npx vitest run`(190件全通過)で検証
- Playwright実機確認: 20倍速で放置し、実際に「レジ行列」の`.live-incident`と
  「本日:在庫そのものが不足」の`.day-alert`が同時表示された状態から、×ボタンで
  両方とも`hidden`属性が付いて非表示になり、店内全体が見えるようになることを
  スクリーンショット・DOM属性の両方で確認。コンソール・ページエラー無し

### 残っている作業

- 「なぜ一時停止しているか」「▶で再開できる」という説明そのものをゲーム内に
  表示する改善(例: 一時停止中はHUDに再開方法を常時表示する等)は今回も
  未着手。閉じるボタンで「見えなくする」ことはできるようになったが、
  一時停止の理由を積極的に説明する導線はまだ弱い(推論: 次にユーザーから
  同種の指摘があれば優先度を上げるべき候補)
- リアルタイム自動停止(`renderLiveIncident()`)と日替わり自動停止
  (`main.ts`の`autoStopForLatestReport()`)が独立した2経路のままである点は
  今回のスコープ外として維持(0-Uのバナー統合に関する記述も参照)

## 0-W. 続くセッション(2026-09-04、倍速設定でもキャラクターの歩行速度がほぼ変わらない問題を修正)

0-Vをマージ・APK再ビルド後、ユーザーから「倍速にしてもキャラの動きが遅いままなんだけど。
ていうか、１倍速でもキャラの動きが遅すぎる」との報告があった。

### 調査

`AskUserQuestion`で希望を確認したところ、「4倍/20倍ボタンはラベルに近い体感速度まで
引き上げてほしい(推奨案を選択)」「1倍の基準速度も少し上げてほしい」という回答を得た。

コードを確認したところ、原因は`visualTimeScale()`(`storeGameRuntime.ts`)にあった。
来店客のスポーン頻度自体は実`Simulation`の実際の加速率(`simMinutesElapsedThisFrame`、
`main.ts`の`SimClock`にそのまま従う)で正しく速くなっていた(0-Sより前の重大バグ修正で
既に対応済み)が、**歩行・接客・補充などCanvas上のアニメーション全般の速度**
(`engine.advance()`へ渡す`deltaSeconds`の倍率)は、選択した速度ラベルよりずっと
控えめな値に据え置かれていた(旧: 20倍選択時でも実質3.2倍、4倍選択時は実質1.9倍)。
「行列に並ぶ客だけどんどん増えるのに、個々のキャラは同じ速度でしか動かない」という
体感の不一致を生んでいた。

### 実施内容

- `visualTimeScale()`: 20倍選択時 3.2→8、4倍選択時 1.9→3.2 へ引き上げ。ラベルの
  倍率そのもの(20倍なら20倍)にはしない — 歩行が一瞬でテレポートしたように見え、
  何が起きているか読み取れなくなるため、以前よりはっきり体感できるが依然として
  目で追える範囲の倍率とした
- `storeOperationsEngine.ts`の`CUSTOMER_MOVE_SPEED`(3.2→4.0)・`STAFF_MOVE_SPEED`
  (3.8→4.8)を引き上げ、1倍速の基準移動速度自体も底上げした(店員が客よりやや
  速いという既存の比は維持)
- いずれもテストで数値を直接検証している箇所は無かった(grep確認済み)ため、
  既存のvitestは無変更で通過。`npm run balance:ci`もCanvas描画層の定数のみの
  変更で実`Simulation`には触れないため、数値は変更前と完全一致
- Playwright実機確認: 1倍速で400ms間隔の2枚のスクリーンショットでは小さな移動、
  20倍速で同じ400ms間隔では時刻が09:15→14:15(約5時間分)進み、客の入れ替わり・
  レジ行列の形成(8人待ち、重大インシデントバナー表示、自動一時停止の発火)まで
  はっきり確認できた。狙い通り速度設定ごとの体感差が明確になったことを確認

### 検証

- `npx tsc --noEmit -p .`: エラー無し
- `npx vitest run`: 191件全通過(既存190件+新規1件、0-V時点から変化なしのため
  新規テストは追加していない — 定数調整のみで振る舞いの分岐は増えていないため)
- `npm run balance:ci`: 変更前と数値的に完全一致
- Playwright: 上記の通り1倍速/20倍速で明確な体感差を確認。コンソール・
  ページエラー無し

### 残っている作業

- 4倍/20倍とも依然としてラベルの倍率そのものではない(8倍・3.2倍という
  中間的な値)。ユーザーが実機で試してまだ遅く感じる場合は、さらに引き上げる
  余地がある(今回はテレポート感とのバランスを見て控えめな値を選んだ、という
  判断であり、確定的な最適値ではない)
- CUSTOMER_MOVE_SPEED/STAFF_MOVE_SPEEDの引き上げは全ての速度設定(1倍含む)に
  等しく効くため、レジ処理速度(`BASE_CHECKOUT_SECONDS`等)や接客ペースとの
  相対バランスは目視確認していない(推論: 極端な破綻は無いとPlaywrightの
  スクリーンショットからは見えるが、厳密な数値バランス検証は行っていない)

## 0-X. 続くセッション(2026-09-04、重大問題での自動一時停止をデフォルトOFFに変更)

0-Wをマージ・APK再ビルド後、ユーザーから「赤い吹き出しが出るたびにいちいち
一時停止しないで」との指示があった。

### 調査

「重大問題を検出したら自動停止」機能自体は既存(README.mdにも記載のある正式機能)
で、時間パネルのチェックボックスでON/OFFできる設計だった。`index.html`を確認した
ところ、この機能の実体である`#auto-stop-checkbox`(`main.ts`の隠しダッシュボード内、
Canvas側の時間パネルの見た目上のチェックボックスとは双方向に同期される — 詳細は
0-V参照)が`checked`属性付きでハードコードされており、**チェック状態はどこにも
永続化されず、ページを開き直すたびに強制的にON へ戻る**作りだった。ユーザーが
以前オフにしていたとしても、APK更新やアプリ再起動のたびに実質リセットされて
しまい、「毎回いちいち止まる」という体感になっていたと考えられる。

### 実施内容

- `index.html`の`#auto-stop-checkbox`から`checked`属性を削除し、既定でOFFにした。
  機能自体(`renderLiveIncident()`のリアルタイム自動停止、`main.ts`の
  `autoStopForLatestReport()`の日替わり自動停止)は削除しておらず、時間パネルの
  チェックボックスから引き続きONにできる
- `npx tsc --noEmit -p .`・`npx vitest run`(191件全通過、既存のまま。この
  チェックボックスのdefault checked状態を直接検証するテストは元々無かった)
- Playwright実機確認: ページ読み込み直後に`#auto-stop-checkbox`・Canvas時間
  パネルの`[data-time-auto-stop]`とも`checked: false`であること、20倍速で
  放置して実際に`.live-incident`が`critical`severityで表示された状態でも
  `#play-button`が「■ 停止」(=再生中)のまま維持され続けることを確認。
  コンソール・ページエラー無し

### 残っている作業

- ユーザーが「重大問題での自動停止」自体を今後使いたくなった場合、時間パネルの
  チェックボックスを毎回手動でONにする必要がある(状態が永続化されないのは
  今回変更していない既存仕様のまま)。もし「オンにしたらその選択を覚えていて
  ほしい」という要望が出れば、localStorage等への永続化を追加で検討する
