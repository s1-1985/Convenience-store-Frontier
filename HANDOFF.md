...
### 2.3 歩行アニメーション → 基盤部分は解消済み(2026-09-02)、実際のコマ絵はまだ

2026-09-02の続く同日セッション(下記0-E参照)で、フレームアニメーションの仕組み
自体(エージェントごとのアニメーション状態、`drawAgentArtwork`のフレーム番号込み
セル選択)を実装した。ただし**現状は`*_FRAMES_PER_DIRECTION`が両ロールとも1の
ままで、見た目は従来と完全に同じ**(仕組みだけが用意された状態)。理由は、この
セッションではPlaywrightによる実機確認ができず、ChatGPT製の4方向×3コマ歩行差分
シートを実際に切り出して組み込む作業(2.1と同種の視覚確認依存作業)を安全に
検証できなかったため。ユーザーへ確認し、切り出し作業ではなくアニメーション基盤の
実装へ切り替えた。

**残っている作業**(実機確認できるセッションで):
- ChatGPT製の4方向×3コマ歩行差分シート(`art-source/chatgpt-adopted-v1/
  characters/base/コンビニ客の歩行スプライトシート.png`等)を切り出し、
  `customers.png`/`staff.png`へ統合する(2.1と同じ手順)
- `src/ui/storeArtAssets.ts`の`CUSTOMER_FRAMES_PER_DIRECTION`/
  `STAFF_FRAMES_PER_DIRECTION`を実際のコマ数へ引き上げる(これだけで
  `drawAgentArtwork`のフレーム選択ロジックがそのまま動き出す設計)
- Playwrightまたは実デバイスでの目視確認(このセッションでは未実施)

旧内容(参考): ChatGPT製キャラクターアセットには4方向×3コマの歩行差分が
含まれているが、旧来のゲームには**フレームアニメーションの仕組みが一切無かった**
(`drawAgentArtwork`は方向ごとに静止1コマを描画するのみ)。実装には
エージェントごとのアニメーションタイマー/現在フレーム、型への状態追加、
セル選択ロジックの変更、アトラス側の1方向1コマ→Nコマ拡張が必要 ―― という
当初の見立て通りに実装した(アトラス拡張部分のみ実素材待ちで未完了)。
