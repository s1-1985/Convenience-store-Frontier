...
演出も未着手 → レジ離脱は0-Rで一部解消。

## 0-R. 続くセッション(2026-09-04、レジ離脱時の吹き出し表示漏れを修正)

「続けて」の指示を受け、0-Qの残課題(客のレジ離脱時の演出が無い)を調査した
ところ、**既に実装済みだが1ケース漏れていた**ことが判明した。`drawCustomer()`
(`storeGameRuntime.ts`)には元々、退店理由(`customer.reason`)に応じて客の頭上へ
吹き出しを出す仕組みがあり、`stockout`(品切?)と`price`(高い…)の2ケースは
既にカバーされていたが、**`queue_abandonment`(レジ待ちに耐えられず退店)だけ
吹き出しが無かった**。この理由は`kpis.queueAbandonments`・「販売機会の損失」
アラート等でも使われる、体感的には最も頻発する退店理由であるにもかかわらず
画面上には一切現れていなかった。

- `drawCustomer()`に`customer.reason === "queue_abandonment"`時の吹き出し
  「待てない」を1行追加(既存のstockout/priceケースと全く同じパターン)
- 純粋なCanvas描画層の1行追加のため新規テストは追加していない(既存の
  stockout/priceケースにもテストが無く、様式を踏襲)。`npx tsc --noEmit -p .`・
  `npx vitest run`は188件全通過のまま変化なし
- Playwrightで実機確認。行列から複数人が同時に「待てない」の吹き出しを
  出しながら退店する様子と、「販売機会の損失 20人が購入せず退店しました」
  という既存の警告バナーが同時に表示され、両者が一致することをスクリーン
  ショットで確認

「見た目・演出が単調」への対応は、会計演出・天候・レジ離脱吹き出しの3点。
入店演出・効果音は引き続き未着手(推論: 効果音は音声アセット基盤自体が
無いため、着手するなら先にアセット調達方針の判断が必要)。

## 0-S. 続くセッション(2026-09-04、「時計を見る客」演出を追加)

「続けて」の指示を受け、`docs/backlog.md` Milestone 7「店内可視化」P2を確認した
ところ、「時計を見る客」「商品を探す客」「廃棄演出」の3項目が未実装のまま
明記されていた。このうち「廃棄演出」は`StoreOperationsEngine`が期限切れ在庫
そのものを一切モデル化していない(廃棄は実`Simulation`側だけの概念)ため
本格着手にはより大きな設計判断を要すると判断し、既存の仕組みを1段階拡張
するだけで実現できる「時計を見る客」に着手した。

- `drawCustomer()`(`storeGameRuntime.ts`)のレジ待ち客吹き出しを、従来の
  「残り6秒未満で"!"」の単一段階から、「残り10秒未満で"⏰"、6秒未満で"!"」
  の2段階へ拡張。使うデータ(`patienceRemainingSeconds`)・スキーマとも
  既存のまま、新しい絵素材も不要
- `npx tsc --noEmit -p .`・`npx vitest run`(188件全通過、変化なし)で検証
- Playwright: 絵文字「⏰」自体は単独の描画テストで正しく表示されることを
  確認し、後続の「残り6秒未満"!"」吹き出しが実プレイで実際に表示される
  こともスクリーンショットで確認した。**ただし「⏰」が出る6〜10秒の窓は
  持続時間が短く、複数回試行したがスクリーンショットのタイミングでは
  直接は捉えられなかった**。ロジックは既存の"!"条件と全く同じ構造の
  数値比較であり、単独の絵文字描画テストと合わせて動作を推論しているが、
  この一点は目視未確認のまま(事実と推論を区別して記録)

「見た目・演出が単調」への対応は、会計演出・天候・レジ離脱吹き出し・
時計を見る客の4点。入店演出・効果音・廃棄演出は引き続き未着手。

## 0-T. 続くセッション(2026-09-04、「客の入店演出」を追加)

「画像関係以外を進めて」との指示を受け、HANDOFF.md §2.7の残課題のうち画像アセットを
伴わない項目として「客の入店演出」(docs/backlog.md Milestone 7 P2)に着手した。
`spawnCustomer()`(storeOperationsEngine.ts)は客を`layout.entranceTile`へ置いて
歩き出させるだけで、画面上には「今まさに入店した」ことを示す合図が一切無かった。

- `storeGameRuntime.ts`に、直前フレームに居なかった客IDを検出する純粋関数
  `newlyArrivedCustomerIds()`と、それを使う`updateEntranceRipples()`/
  `drawEntranceRipple()`を追加。新規の客IDを検出すると、その客の足元へ
  500ms かけて広がりながら消える楕円のリング(`#ffe9a8`)を描く。会計演出
  「+¥N」(0-P)と同じ設計方針(エンジン側のスキーマ変更なし、Canvas描画層限定)。
  セーブ直後の初回フレーム(客が既に店内にいる状態の復元)で誤発火しないよう、
  `floatingSaleTexts`と同じ「初回フレームはスキップ」ガードを入れている
- `drawCustomer()`/`drawStoreContents()`/`drawFrame()`の呼び出し鎖に`timestamp`を
  通し(既存の`drawRainOverlay`と同じ経路)、`render()`内で`updateEntranceRipples()`を
  `drawFrame()`呼び出し前に実行するよう配線した
- `src/tests/storeGameShell.test.ts`に1本追加(計6本、他ファイルと合わせ全体189件
  全通過)。この`storeGameRuntime.ts`はモジュール全体が非公開(exportなし、
  副作用で自己初期化する設計)のため、既存の慣例どおりソーステキストへの
  トークン存在チェックによるスモークテストとした
- `npx tsc --noEmit -p .`・`npx vitest run`(189件全通過)・`npm run balance:ci`
  (Canvas描画層のみの変更のため数値は変更前と完全一致)で検証
- Playwright実機確認: スクリーンショット(250ms間隔)ではリップルの500msという
  短い表示窓を直接捉えられなかったため(HANDOFF 0-Sの「⏰」表示と同じ制約)、
  録画(`recordVideo`)からのフレーム抽出、および`CanvasRenderingContext2D.
  prototype.ellipse`をフックして`strokeStyle === "#ffe9a8"`(リップル固有の色)
  での呼び出し回数を計測する方法を追加で用いた。開店直後から実際の客が
  スポーンする窓で320回の該当描画呼び出しを確認し、実プレイ中にコードパスが
  実行されていることをプログラム的に確認した(視覚的な「リングがそれと分かる形で
  見えるか」の目視確認自体は、上記と同じ理由で完了していない)

### 残っている作業

- リップルの見た目(色・太さ・持続時間)は初期値の当てずっぽうで、実際に
  目視で「入店した」と分かりやすいかは未調整。次に実機確認できるセッションで
  微調整するとよい
- HANDOFF.md §2.7の「客の入店演出」は着手済みに更新(下記2.7参照)。効果音・
  「商品を探す客」・「廃棄演出」は引き続き未着手
...
- ~~`set_category_area`/`set_task_priorities`をCanvas側から即時反映すること~~
  → 2.6参照、解消済み
- ~~通知に原因階層を持たせる(`docs/game-design.md`10節)~~ →
  フェーズ2f(0-O参照)で着手済み。ただし現状は1段階のみ・既存
  `DailyReport`フィールドのみを使った範囲にとどまる(カテゴリ・時間帯レベルの
  粒度は`buildDashboardAlerts()`のシグネチャ変更を伴うため未着手)
- ~~当日のライブな天候(`weather`)をCanvas上に反映する~~ → フェーズ2g
  (0-Q参照)で着手済み。`SimulationSnapshot.weather`を公開し、HUD表示と
  雨天時のティント・雨筋オーバーレイに反映した
...
### 2.7 「見た目・演出が単調」への対応(新規、2026-09-03〜04、0-N以降)

ユーザーへプレイ画面のスクリーンショットを見せたところ「ゲームになる気がしない」
「見た目・演出が単調」「目標・緊張感が弱い」との率直な感想があり、以降のセッションで
継続的に対応した一連の作業。指示は「特に指示は無いので、アイデアで進めるか、
わからなければask」というスタンスで、以下は基本的に自主判断で実施したもの。

**着手済み**:
- 日次経営診断をCanvas上に表示(0-N、フェーズ2e)
- 通知の原因階層(0-O、フェーズ2f)
- 会計成立時の演出「浮遊+¥N」(0-P)
- 天候の実況反映・雨天オーバーレイ(0-Q、フェーズ2g)
- レジ離脱時の吹き出し表示漏れ修正(0-R)
- 「時計を見る客」演出(0-S、`docs/backlog.md` Milestone 7 P2記載項目)
- 客の入店演出(0-T、`docs/backlog.md` Milestone 7 P2記載項目)

**未着手のまま(次セッションへの引き継ぎ)**:
- 効果音(音声アセット基盤自体が無い。着手するなら先にアセット調達方針の
  ユーザー判断が必要)
- `docs/backlog.md` Milestone 7 P2の残り2項目:「商品を探す客」「廃棄演出」
  (廃棄演出は`StoreOperationsEngine`が期限切れ在庫を一切モデル化していない
  ため、本格着手には設計判断が必要)
- 0-Sで追加した「⏰」(時計を見る客、残り10秒未満)表示は、ロジック・絵文字
  描画とも個別には確認済みだが、6〜10秒の窓が短く実プレイでの目視確認が
  未完了のまま(PR #85参照)。0-Tの入店リップル(500ms)も同じ理由で
  目視確認は未完了(Canvas API呼び出しのプログラム的確認のみ済み)
...
- `docs/visual-numeric-engine-integration.md` — 見た目Canvasと数値エンジンの統合状況(フェーズ1完了、フェーズ2a〜2g着手済み・残課題は同ドキュメント参照)
